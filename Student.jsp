...
<body>
	...
	<% 
		...
		if(status){
		String msgSymbol=	"<a href='#' id='getStudentNotification'>"+
			"<img src='images/msg.ico' title='You have a message' alt='Incoming Message' width='52' height='52' style='padding-left: 20px;padding-top: 20px;padding-right: 20px;' border='0'/></a>";
	...
	<!--  	
			<a href="#">
					<img src="images/msg.ico" title="You have a message" alt="Incoming Message" width="52" height="52" style="padding-left: 20px;padding-top: 20px;padding-right: 20px;" border="0"/></a>
	-->
	<button class="myBtn" id="searchTeacher">Search Teacher</button>
	<button class="myBtn" id="stdAlltchr">My Teachers</button>
		<div id='info'>
		...
		<%
			out.println("Logged in user : "+((User)session.getAttribute("user")).getUserName()+" || ");
			out.println("Role : "+((User)session.getAttribute("user")).getRole());
		%>
		</div>
		</div>
